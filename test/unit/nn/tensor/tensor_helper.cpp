...
TEST(TensorHelperTest, IncrementIndices) {
  TensorIndices indices(3, 0);
  TensorShape shape({2, 3, 2});

  EXPECT_TRUE(indices == TensorIndices({0, 0, 0}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  ...
  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({0, 1, 1}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({0, 2, 0}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({0, 2, 1}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({1, 0, 0}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({1, 0, 1}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({1, 1, 0}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({1, 1, 1}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({1, 2, 0}));

  EXPECT_TRUE(TensorHelper::increment_indices(indices, shape));
  EXPECT_TRUE(indices == TensorIndices({1, 2, 1}));
